...
main = do
    runHEPGlobal $! H.proc $! do
        prog <- startProgram "setsid" ["ssh", "-n", "testuser@192.168.2.21", "hproxyserver -d 192.168.2.101:10000"] 
            (\str -> do
                liftIO $! print $! "out: " ++ unpack str
                return ()
            ) 
            (\str -> do
                liftIO $! print $! "err: " ++ unpack str
                return ()
            ) 
        liftIO $! print "write pass\n"
        --writeInput prog "Qwer1234\n"
        procFinished
    return ()
...
startProgram:: String -- cmd
            ...
            -> HEP Pid
startProgram cmd args onRead onError = do
...
readerWorker h onRead = do
    ...
    case read of
        0 -> procFinished
        _ -> do
            liftIO $! Sys.putStrLn $! "readed " ++ show read
